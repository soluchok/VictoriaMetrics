...
func (wr *writeContext) appendSamplesFromExponentialHistogram(metricName string, p *pb.ExponentialHistogramDataPoint) {
	...
	wr.appendSample(metricName+"_sum", t, *p.Sum, isStale)
	rate := math.Pow(2, -float64(p.Scale))
	base := math.Pow(2, rate)
	lowerBound := 1.0
	upperBound := base
	if p.ZeroCount > 0 {
	...
	if p.Positive != nil {
		upperBound = math.Pow(2, float64(p.Positive.Offset+1)*rate)
	...
	if p.Negative != nil {
		lowerBound = math.Pow(2, -float64(p.Negative.Offset+1)*rate)
