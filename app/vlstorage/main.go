...
var (
	...
	futureRetention = flagutil.NewDuration("futureRetention", "2d", "Log entries with timestamps bigger than now+futureRetention are rejected during data ingestion; "+
		"see https://docs.victoriametrics.com/VictoriaLogs/#retention")
	storageDataPath = flag.String("storageDataPathVl", "victoria-logs-data", "Path to directory with the VictoriaLogs data; "+
	...
	minFreeDiskSpaceBytes = flagutil.NewBytes("storage.minFreeDiskSpaceBytesVl", 10e6, "The minimum free disk space at -storageDataPath after which "+
